...
import {
    DexLiteSlotsLink as DSL,
    AddressCalcs
} from "libraries/FluidLiteLib.sol";
...
contract FluidLiteQuote {
    ...
    function getAllDexKeys() external view returns (DexKey[] memory dexKeys_, bytes8[] memory dexIds_) {
        uint256 length_ = _readStorage(DSL.DEX_LITE_DEXES_LIST_SLOT);
        bytes32 dataSlot_ = keccak256(abi.encode(DSL.DEX_LITE_DEXES_LIST_SLOT));
    ...
    function getLiquidityByKey(DexKey memory dexKey_) public view returns (uint256 centerPrice_, uint256 token0ImaginaryReserves_, uint256 token1ImaginaryReserves_) {
        bytes8 dexId_ = calculateDexIdByKey(dexKey_);
        uint256 dexVariables_ = _readMappingStorage(DSL.DEX_LITE_DEX_VARIABLES_SLOT, dexId_);

        if (dexVariables_ == 0) {
            revert("Dex not initialized");
        }

        uint256 token0Supply_ = (dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED) & (2**60 - 1);
        uint256 token1Supply_ = (dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_1_TOTAL_SUPPLY_ADJUSTED) & (2**60 - 1);

        return _getPricesAndReserves(dexKey_, dexVariables_, dexId_, token0Supply_, token1Supply_);
    }

    function getKeyAndLiquidityById(bytes8 dexId_) external view returns (DexKey memory dexKey_, uint256 centerPrice_, uint256 token0ImaginaryReserves_, uint256 token1ImaginaryReserves_) {
        dexKey_ = getDexKey(dexId_);
        (centerPrice_, token0ImaginaryReserves_, token1ImaginaryReserves_) = getLiquidityByKey(dexKey_);
    }
    ...
    function _readMappingStorage(uint256 baseSlot_, bytes8 key_) internal view returns (uint256 value_) {
        bytes32 slot_ = DSL.calculateMappingStorageSlot(baseSlot_, key_);
    ...
    function _getPricesAndReserves(
        ...
        if (temp_ > 1) {
            unchecked {
                ...
                if (temp_ == 2) {
                    temp2_ = _readMappingStorage(DSL.DEX_LITE_CENTER_PRICE_SHIFT_SLOT, dexId_); // _centerPriceShift[dexId_]
    ...
    function _calcCenterPrice(
        ...
        uint256 dexVariables_,
        bytes8 dexId_
    ) internal view returns (uint256 newCenterPrice_) {
        ...
        uint256 centerPriceShift_ = _readMappingStorage(DSL.DEX_LITE_CENTER_PRICE_SHIFT_SLOT, dexId_); // _centerPriceShift[dexId_]
    ...
    function _calcRangeShifting(
        ...
        uint256 rangeShift_ = _readMappingStorage(DSL.DEX_LITE_RANGE_SHIFT_SLOT, dexId_); // _rangeShift[dexId_]
