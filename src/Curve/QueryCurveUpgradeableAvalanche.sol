...
import {QueryCurveUpgradeable, ICurveV2Pool, ICurvePool, ICurveNGPool, TokenInfo, IERC20} from "./QueryCurveUpgradeable.sol";
...
contract QueryCurveUpgradeableAvalanche is UUPSUpgradeable {
    ...
    function initialize2(address registry) reinitializer(2) onlyProxy public {
        // 这里之所以再设置一遍是因为proxy需要，否则proxy里面这个属性就是0x0
        meta_registry = registry;
    }
    ...
    function get_balances(
        ...
        if (pool == 0x7f90122BF0700F9E7e1F688fe926940E8839F353 || 
    ...
    function get_tokens_with_decimals(
        address pool
    ) public view returns (TokenInfo[8] memory tokenInfos) {
    ...
    function set_meta_registry(address registry) onlyOwner public {
        meta_registry = registry;
    }
